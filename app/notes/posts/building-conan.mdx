...
summary: 'I like dashboards. So when everything Claude Code does kept vanishing into scrollback, I built one worth staring at. Somewhere in the mess of shipping it, the agent ended up making its own launch video.'
---
...
<PullQuote from="From the brief, to myself">
...
<Margin label="The 60% rule">
  <LineChart
    label="Agent reliability slips gently until ~60% of context, then drops off sharply"
  ...
  />
  <Caption>Context rot: as an agent's context window fills, its reliability decays, slipping from the first tokens, then accelerating past the 60% line.</Caption>
</Margin>
